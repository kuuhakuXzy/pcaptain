...
if [ -n "$PCAP_ROOT_DIRECTORIES" ]; then
  yq -i '.pcap.root_directories = (env(PCAP_ROOT_DIRECTORIES) | split(",") | map(select(length > 0)))' "$CONFIG_FILE"
fi
...
if [ -n "$QUICK_SCAN_SAMPLE_SEGMENTS" ]; then
  yq -i '.pcap.quick_scan.sample_segments = env(QUICK_SCAN_SAMPLE_SEGMENTS)' "$CONFIG_FILE"
fi

if [ -n "$MAX_PARALLEL_SCANS" ]; then
  yq -i '.pcap.max_parallel_scans = env(MAX_PARALLEL_SCANS)' "$CONFIG_FILE"
fi
