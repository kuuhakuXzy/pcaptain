...
from services.catalog_stats import (
    build_stats_summary,
    get_co_occurrence_for_protocol,
    get_top_talkers,
)
...
@router.get("/stats/top-talkers", summary="Top IPs and ports by indexed file count")
async def stats_top_talkers(
    ...
    if not redis:
        raise HTTPException(503, "Redis unavailable")
    return await get_top_talkers(redis, top=top)
