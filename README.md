...
By default, the host folder `./pcaps` is mounted into the container at `/pcaps`. You can point `PCAP_DIRECTORIES` at any folder on your host (including a Windows junction or symlink).

**Single custom mount**

```env
PCAP_DIRECTORIES=./my-archive
PCAP_MOUNTED_DIRECTORY=/pcaps
```
...
The search UI **Config** button (gear icon) shows the active host and container paths via `GET /pcap-config`.

**Multiple mounts**

Mount several host folders and list their container paths in `PCAP_ROOT_DIRECTORIES`:
...
```env
PCAP_ROOT_DIRECTORIES=/pcaps/archive,/pcaps/lab
PCAP_PREFIX_STR=./archive,./lab
```

Scanning walks every configured root. Use **Scan → Choose Folder** in the UI (or `POST /reindex/{relative_path}`) to index one subfolder only.

After changing mounts, run a full **Scan → All** so Redis paths stay in sync with the new layout.
...
-   **Re-scan a specific subfolder (relative path under a PCAP root):**
    ```bash
    curl -X POST "http://localhost:8000/reindex/my-subfolder"
    ```

-   **Get mount directory configuration:**
    ```bash
    curl http://localhost:8000/pcap-config | jq
    ```

-   **List scannable subfolders:**
    ```bash
    curl http://localhost:8000/pcap-folders | jq
    ```
